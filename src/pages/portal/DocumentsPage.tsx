...
export default function DocumentsPage() {
  ...
  const handleUpload = async () => {
    ...
    const { error } = await supabase.from("documents").insert({ title, category, file_url: urlData.publicUrl, uploaded_by: profile.full_name });
    if (error) toast.error(error.message);
    else { toast.success("Uploaded!"); setOpen(false); setTitle(""); setCategory("Other"); setFile(null); fetchDocs(); }
    setUploading(false);
  };
